...
namespace S2Geometry.Tests
{
  ...
  new TestCase(0, 0, 0.0, 10.0, new Chain[] {new Chain(null, false)}, new String[] {}, 0),
      ...
      new TestCase(0, 1, 1.5, 5.8, // XOR, min_merge > sqrt(2), max_merge < 6.
        ...
        bool testBuilder(TestCase test) {
    for (int iter = 0; iter < 200; ++iter) {
      ...
      foreach (Chain chain in test.chainsIn) {
      ...
      List<S2Loop> expected = new List<S2Loop>();
      foreach (String loop in test.loopsOut) {
        List<S2Point> vertices = new List<S2Point>();
    ...
    return true;
        ...
        public void testAssembleLoops()
        {
            ...
            for (int i = 0; i < testCases.Length; ++i)
            {
                ...
                bool caseSuccess = testBuilder(testCases[i]);
