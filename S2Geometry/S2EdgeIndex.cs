...
namespace Google.Common.Geometry
{
  ...
  private void sortIndex() {
    ...
    ulong[] newCells = new ulong[cells.Length];
  ...
  private void getEdgesInParentCells(List<S2CellId> cover, HashSet<int> candidateCrossings) {
    ...
    HashSet<S2CellId> parentCells = new HashSet<S2CellId>();
    foreach (S2CellId coverCell in cover) {
  ...
  private void getEdgesInChildrenCells(S2Point a, S2Point b, List<S2CellId> cover,
    ...
    while (cover.Any())
    {
        S2CellId cell = cover[cover.Count - 1];
      ...
      if (bounds[1] - bounds[0] <= 16) {
        ...
        foreach (S2Cell child in children) {
